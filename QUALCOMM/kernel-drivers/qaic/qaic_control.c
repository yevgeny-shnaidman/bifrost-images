...
{
	struct wire_trans_validate_part_from_dev *trans_rsp;
	struct wire_trans_validate_part_to_dev *trans_req;
	struct wire_msg *msg_req, *msg_rsp;
	struct wrapper_list *wrappers;
	struct wrapper_msg *wrapper;
	size_t msg_rsp_len;
	int ret = 0;
	...
	wrappers = alloc_wrapper_list();
	if (!wrappers) {
		trace_qaic_manage(usr->qddev, "Request dev partition, failed to allocate wrapper list.",
				  -ENOMEM);
		return -ENOMEM;
	}

	wrapper = add_wrapper(wrappers, sizeof(*wrapper) + sizeof(*msg_req) + sizeof(*trans_req));
	if (!wrapper) {
		kfree(wrappers);
		trace_qaic_manage(usr->qddev, "Request dev partition, failed to allocate wrapper.",
				  -ENOMEM);
		return -ENOMEM;
	}
	...
	trans_req = (struct wire_trans_validate_part_to_dev *)msg_req->data;
	trans_req->hdr.type = cpu_to_le32(QAIC_TRANS_VALIDATE_PARTITION_TO_DEV);
	trans_req->hdr.len = cpu_to_le32(sizeof(*trans_req));
	trans_req->part_id = cpu_to_le32(args->partition_id);
	...
	if (IS_ERR(msg_rsp)) {
		ret = PTR_ERR(msg_rsp);
		trace_qaic_manage(usr->qddev, "Request dev partition, message transfer failed.",
				  ret);
		goto kfree_wrapper;
	}

	msg_rsp_len = sizeof(msg_rsp->hdr) + sizeof(*trans_rsp);
	if (le32_to_cpu(msg_rsp->hdr.count) != 1 || le32_to_cpu(msg_rsp->hdr.len) < msg_rsp_len) {
		ret = -EINVAL;
		trace_qaic_manage_2(usr->qddev, "Invalid response received for dev partition. message count %llu message len %llu",
				    le32_to_cpu(msg_rsp->hdr.count), le32_to_cpu(msg_rsp->hdr.len));
		goto kfree_msg_rsp;
	}

	trans_rsp = (struct wire_trans_validate_part_from_dev *)msg_rsp->data;
	if (le32_to_cpu(trans_rsp->status)) {
		trace_qaic_manage_1(usr->qddev, "Dev partition request failed status is %llu.",
				    le32_to_cpu(trans_rsp->status));
		ret = -EPERM;
	}
