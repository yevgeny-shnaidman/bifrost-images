...
#ifdef _QBP_NEED_XARRAY_STABLE_XA_ALLOC
#include <linux/idr.h>
#else
...
#ifdef _QBP_NEED_XARRAY_STABLE_XA_ALLOC
static DEFINE_IDR(mqc_xa);
static DEFINE_MUTEX(mqc_xa_lock);
#else
...
#ifdef _QBP_NEED_XARRAY_STABLE_XA_ALLOC
	int ret;
	ret = mutex_lock_interruptible(&mqc_xa_lock);
	if (ret)
		return NULL;
	mqcdev = idr_find(&mqc_xa, minor);
	if (mqcdev)
		kref_get(&mqcdev->ref_count);
	mutex_unlock(&mqc_xa_lock);
#else
	xa_lock(&mqc_xa);
...
#ifdef _QBP_NEED_XARRAY_STABLE_XA_ALLOC
	mutex_lock(&mqc_xa_lock);
	mqcdev->minor = idr_alloc(&mqc_xa, mqcdev, 0,
				  MHI_QAIC_CTRL_MAX_MINORS, GFP_KERNEL);
	mutex_unlock(&mqc_xa_lock);
	if (mqcdev->minor < 0) {
		ret = -ENOMEM;
		kfree(mqcdev);
...
#ifdef _QBP_NEED_XARRAY_STABLE_XA_ALLOC
		mutex_lock(&mqc_xa_lock);
		idr_remove(&mqc_xa, mqcdev->minor);
		mutex_unlock(&mqc_xa_lock);
#else
